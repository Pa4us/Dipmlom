...
namespace WebAPI.Infrastructure
{
    ...
    public static class DbSeeder
    {
        public static async Task SeedAsync(IServiceProvider serviceProvider)
        {
            ...
            // ── Блоки и комнаты: добавляем недостающие при каждом запуске ────
            await EnsureBlocksAndRoomsAsync(db);
            ...
            var roleMechanic  = new Role { Name = "Mechanic",   Description = "Мастер (выполняет заявки на ремонт)" };
            ...
            // ── 4 и 5. Блоки и комнаты уже созданы EnsureBlocksAndRoomsAsync ─
            // Получаем нужные для seed-данных блоки по номеру
            var b41 = await db.Blocks.FirstAsync(b => b.BlockNumber == "41");
            var b42 = await db.Blocks.FirstAsync(b => b.BlockNumber == "42");
            var b43 = await db.Blocks.FirstAsync(b => b.BlockNumber == "43");

            var r401 = await db.Rooms.FirstAsync(r => r.BlockId == b41.Id && r.RoomNumber == "1");
            var r402 = await db.Rooms.FirstAsync(r => r.BlockId == b41.Id && r.RoomNumber == "2");
            var r403 = await db.Rooms.FirstAsync(r => r.BlockId == b42.Id && r.RoomNumber == "1");
            var r405 = await db.Rooms.FirstAsync(r => r.BlockId == b43.Id && r.RoomNumber == "1");
        ...
        /// Гарантирует наличие всех 27 блоков (9 этажей × 3 блока) и 54 комнат (2 на блок).
        /// Безопасно вызывать при каждом запуске — добавляет только отсутствующие блоки.
        /// Формат BlockNumber: «{этаж}{индекс_блока}» → 11, 12, 13, 21 … 91, 92, 93.
        private static async Task EnsureBlocksAndRoomsAsync(AppDbContext db)
        {
            var existingNumbers = await db.Blocks
            ...
            for (int floor = 1; floor <= 9; floor++)
            {
                for (int idx = 1; idx <= 3; idx++)
                {
                    ...
                    if (!existingNumbers.Contains(num))
                        newBlocks.Add(new Block { BlockNumber = num, Floor = floor, BlockIndex = idx });
                }
            }
            ...
            // Добавляем по 2 комнаты к каждому новому блоку
            foreach (var block in newBlocks)
            {
                db.Rooms.Add(new Room { RoomNumber = "1", BlockId = block.Id, Capacity = 2, CurrentOccupancy = 0, IsActive = true });
                db.Rooms.Add(new Room { RoomNumber = "2", BlockId = block.Id, Capacity = 2, CurrentOccupancy = 0, IsActive = true });
            }
            await db.SaveChangesAsync();
        }
